...
<table id="tabelSepatu2" class="table table-sm table-striped table-bordered pr-2" cellspacing="0" width="100%">
    <thead>
        <tr>
            ...
            <th class="th-sm text-center t_order1_col1">Kode Kategori</th>
            <th class="text-center t_order1_col0">Nama Kategori</th>
            <th class="text-center t_order1_col0">Diskon</th>
    ...
    @foreach ($model as $item)
        <tr class="">
            ...
            <td class="mr-0 py-1 ">{{ $item->kode_kate}}</td>
            <td class="mr-0 py-1 ">{{ $item->namakate }}</td>
            <td class="mr-0 py-1 ">{{ $item->diskon}}</td>
            ...
            <td class=" text-align-center  mr-0 py-1 ">
                    ...
                    <a class="btnEdit btn btn-info text-center btn btn-info  btn-sm  mr-0 py-1 "
                        kode="{{ $item->kode_kate }}">Edit</a>
                    @csrf
                    <button type="button" kode="{{ $item->kode_kate  }}"
...
<script>
      ...
      $(document).ready(function() {
            const kate = '{"1": "Gradial", "2": "Gardial2", "3": "Gardial3"}';
            $.ajaxSetup({
                headers: {
        ...
        $(document).on('click', '.btnEdit', function (e) {
            ...
            console.log(kode);
            var baseurl = '{{url('/')}}';
            $.get(baseurl+'/{{$url}}/kategoriEdit_'+kode, function (data) {
                console.log(data);
                $('#div1').hide();
                $('#div2').html(data);
                $('#div3').hide();
