...
@section('JS')
    <script>
        ...
        $(document).on('click', '#btnTambah' , function(e){
            ...
            console.log("fsdfdsf");
            var baseurl = '{{ url('/')}}';
            $.get(baseurl+'/{{$url}}/sepatu1Add', function(data){
        ...
        $(document).on('click', '#btnSave', function(e) {
            var isvalidate = $("#formAddsepatu1")[0].checkValidity();

            if (isvalidate) {
                e.preventDefault();
                var formData = new FormData($("#formAddsepatu1")[0]); // khusus form dengan file upload
        ...
        $(document).on('click', '.btnhapus', function(e) {
            ...
            if (confirm("Yakin Anda Ingin Menghapus Data !")) {
                ...
                var baseurl = '{{ url('/') }}';
                $.get(baseurl + '/{{$url}}/sepatu1delete_' + kodedel, function(data) {
                    tabelIndex();
                    // $('#div1').html(data);
                    // $('#div2').hide();
                    // $('#div3').hide();
