...
@section('JS')
    <script>
        ...
        $(document).on('click', '#btnTambah' , function(e){
            ...
            console.log("fsdfdsf");
            var baseurl = '{{ url('/')}}';
            $.get(baseurl+'/{{$url}}/tokoAdd', function(data){
        ...
        $(document).on('click', '#btnSave', function(e) {
            var isvalidate = $("#formAddToko")[0].checkValidity();

            if (isvalidate) {
                e.preventDefault();
                var formData = new FormData($("#formAddToko")[0]); // khusus form dengan file upload
        ...
        $(document).on('click', '.btnhapus', function(e) {
            ...
            if (confirm("Yakin Anda Ingin Menghapus Data !")) {
                ...
                var baseurl = '{{ url('/') }}';
                $.get(baseurl + '/{{$url}}/tokodelete_' + kodedel, function(data) {
                    tabelIndex();
                    // $('#div1').html(data);
                    // $('#div2').hide();
                    // $('#div3').hide();
